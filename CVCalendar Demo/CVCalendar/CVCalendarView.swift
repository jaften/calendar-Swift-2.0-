...
public final class CVCalendarView: UIView {
    ...
    public var view :(weekViewSize: CGSize?, dayViewSize: CGSize?)
...
extension CVCalendarView {
    public func commitCalendarViewUpdate() {
        if let _/*delegate*/ = delegate, let contentController = contentController {
            ...
            if !validated && allowed {
                ...
                if let mode = calendarMode {
                    ...
                    view.weekViewSize = CGSizeMake(width, height)
                    view.dayViewSize = CGSizeMake((width / 7.0) - hSpace, height)
                    validated = true
...
extension CVCalendarView {
    ...
    public func changeMode(mode: CalendarMode) {
        if let selectedDate = coordinator.selectedDayView?.date.convertedDate() where calendarMode != mode {
            ...
            switch mode {
            case .WeekView:
                contentController.updateHeight(view.dayViewSize!.height, animated: true)
